...
export class TarefaService {
  ...
  listar(): void {
    ...
    this.http.get<Tarefa[]>(this.API, { params }).subscribe((tarefas) => {
      ...
      tarefasTemporarias = tarefasTemporarias.concat(tarefas); //concatenando as tarefas que recebemos com as tarefas temporárias
      this.tarefasSubject.next(tarefas); //emitir os novos valores
    });
  }
  ...
  criar(tarefa: Tarefa): void {
    this.http.post<Tarefa>(this.API, tarefa).subscribe((novaTarefa) => {
      ...
      tarefas.unshift(novaTarefa); //adicionando a tarefa no inicio do array, diferente do push - o unshift faz isso
  ...
  excluir(id: number): void {
    ...
    this.http.delete<Tarefa>(url).subscribe((tarefaEditada) => {
      const tarefas = this.tarefasSubject.getValue();
